 ...
 * UI never infers required steps from empty lists — only renders what backend provides.
 */
export default function SelectionList({ list, onSelect, selectedId, disabled }: SelectionListProps) {
  return (
          ...
          >
            <CardContent className="flex items-center gap-2 p-2.5">
              <div className="flex-1 min-w-0">
                ...
                {/* For specialty type, show confidence bar if available */}
                {list.type === "specialty" && item.confidence != null && (
                  <Progress value={item.confidence * 100} className="mt-1 h-1.5" />
                )}
              </div>
              {list.type === "specialty" && item.confidence != null && (
